...
@app.get("/orgs/{org_slug}", response_model=OrgResponse)
async def get_org(org_slug: str):
    org = _ORGS.get(org_slug)
    if org is None:
        raise HTTPException(status_code=404, detail={"error": "org not found", "slug": org_slug})
    return org

@app.get("/orgs/{org_slug}/repos/{repo_id}", response_model=RepoResponse)
async def get_repo(org_slug: str, repo_id: int):
...
@app.get("/files/{file_path}", response_model=FileResponse)
...
async def get_trace(trace_id: UUID):
    return {"trace": str(trace_id), "found": False}