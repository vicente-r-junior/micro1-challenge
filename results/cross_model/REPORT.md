...
|---|---|---|---|---|---|---|
| `v0_baseline` | single prompt (baseline) | **10/16** (62%) | 88.6% | 16 | 0.7978 | 44 min |
| `v2_repair` | + tool-calling repair loop | **16/16** (100%) | 100.0% | 26 | 1.5609 | 100 min |

## Headline

Shippable migrations go from **10/16** with a single prompt to **16/16** with the full agent — +6 cases, +38% of the benchmark.

Mean parity moves 88.6% → 100.0%. The mean moves less than the shippable count, and that is the point: the baseline is already *nearly* right almost everywhere. The value is not in the average, it is in closing the last few probes — which are the ones that break production and which no code review catches.
...
|---|---|---|
| `v0_baseline` | 10/14 | 0/2 |
| `v2_repair` | 14/14 | 2/2 |

## Agent behaviour (`v2_repair`)

- Cases needing repair: 3/16
- Repair turns when used: 2–5 (budget 8)
- Tool calls: `get_probe_detail` ×11, `run_differential` ×3, `search_legacy` ×4
- Lessons in the ledger at the end: 0
- Probes scored on status alone (framework error pages): 55/212
- Concurrency: 2 workers, wave size 16
